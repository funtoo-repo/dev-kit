...
SRC_URI="
	https://api.github.com/repos/abseil/abseil-cpp/tarball/20250512.1 -> abseil-cpp-20250512.1.tar.gz
	test? ( https://api.github.com/repos/google/googletest/tarball/v1.17.0 -> googletest-1.17.0.tar.gz )
"
LICENSE="
	Apache-2.0
	test? ( BSD )
...
fix_src_dirs() {
	...
	mv google-googletest-* googletest-1.17.0
	popd
}
...
src_prepare() {
	...
	if use test; then
		sed -i 's/-Werror//g' \
			"${WORKDIR}"/googletest-1.17.0/googletest/cmake/internal_utils.cmake || die
	fi
}
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DABSL_LOCAL_GOOGLETEST_DIR="${WORKDIR}/googletest-1.17.0"
